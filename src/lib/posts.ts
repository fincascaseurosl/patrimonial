...
export function getPostTitle(post: Post, locale: string): string {
  return locale === "ca" ? post.titleCa || post.titleEs : post.titleEs || post.titleCa;
}

export function getPostExcerpt(post: Post, locale: string): string {
  return locale === "ca" ? post.excerptCa || post.excerptEs : post.excerptEs || post.excerptCa;
}

export function getPostBody(post: Post, locale: string): string {
  return locale === "ca" ? post.bodyCa || post.bodyEs : post.bodyEs || post.bodyCa;
}

export function getPostMetaTitle(post: Post, locale: string): string {
  if (locale === "ca") return post.metaTitleCa || post.titleCa || post.titleEs;
  return post.metaTitleEs || post.titleEs || post.titleCa;
}

export function getPostMetaDescription(post: Post, locale: string): string {
  if (locale === "ca") return post.metaDescriptionCa || post.excerptCa || post.excerptEs;
  return post.metaDescriptionEs || post.excerptEs || post.excerptCa;
}
